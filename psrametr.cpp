...
int main()
{
    ...
    F(0) = bound_left - 2*y(0) + y(1) - h * h * (1-ep)*sin(y(0)) - h * h * ((ep*y(0)/(2*PI*PI)) * (y(0)-2*PI) * (y(0) - PI)) ; // первая минус 1 соответствует у(a_left)
    F(N-1) =  y(N-2) - 2*y(N-1) + bound_right - h * h * (1-ep)*sin(y(N-1)) - h * h * (ep*y(N-1)/(2*PI*PI) * (y(N-1)-2*PI) * (y(N-1) - PI)) ; // первая + 1 соответствует у(b_right)
    
    for(int i = 1; i< N-1; i++)
    {
        F(i) = y(i-1) - 2*y(i) + y(i+1) - h * h * (1-ep)*sin(y(i)) - h * h * (ep*y(i)/(2*PI*PI) * (y(i)-2*PI) * (y(i) - PI)); //FIX POTENTIAL
    }
        
    for(int i = 0; i< N; i++) //индекс 0 соответствует у(a_left + h)
    {
        dg(i) = h*h * (1-ep)*cos(y(i)) + h*h * (ep/(2*PI*PI)) * (3*pow(y(i),2) - 6*y(i)*PI + 2*pow(PI,2)); // FIX POTENTIAL
    }
