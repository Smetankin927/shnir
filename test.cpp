...
float dg (float y)
{
    return ((1-ep) * cos(y*PI/180) + ep * (3*y*y -6*PI*y + 2*PI*PI)/(2*PI*PI));  
}
float f (float y)
{
    return ((1-ep) * sin(y*PI/180) + ep * y * (y - PI) * (y - 2*PI)/(2 * PI * PI)) ;  
}
...
int main()
{
    ...
    {
        //cout<<"2"<<endl;
        //cout<<"solve "<<norm(solve(W, -F), 2)<<endl;
        V = V + solve(W, -F);
        W = calculateW (W, V);
        F = calculateF (F, V);
    ...
    {
        //cout<<"3"<<endl;
        //cout<<"abs = "<<abs(V(N-1) - bound_right)<<endl;
        if(V(N-1) > bound_right)
        {
            lambda = lambda - abs(V(N-1)-bound_right)*0.01;
        }
