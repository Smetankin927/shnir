...
int main()
{
    ...
    F(0) = -1 - 2*y(0) + y(1) - koeff * h * h * 2 * y(0) * (y(0) * y(0) -1); // первая минус 1 соответствует у(a_left) // если фиксировать гранусловия
    F(N-1) =  y(N-2) - 2*y(N-1) + 1 - koeff * h * h * 2 * y(N-1) * (y(N-1) * y(N-1) -1); // первая + 1 соответствует у(b_right)//то фиксить и здесь
...
for(int i =0; i < Iteration; i++)
{
   ...
   for(int i =0; i<N; i++)
   {
    ...
    //запись в файл
    ofstream out;
    out.open("data_anti.txt");
    for(int i=0; i<N+2; i++)
    {if (out.is_open())
    {
        out<< -ansY(i)<<'\t'<< ansX(i) +2 <<endl;
    }
    }
    out.close();
}