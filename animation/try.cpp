...
vec diff(vec v, int N, float h)
{
    ...
    for(int i =2; i<N-2; i++)
    {
        ddv(i) = (- v(i-2) + 16 * v(i-1) - 30 * v(i) + 16 * v(i+1) - v(i+2))/(12*h*h);
    }
    return ddv;
}
...
int calculate_and_write(int Iter)
{
    ...
    //считывание f(0,x) из файла data_anti.txt
    //ifstream in;
    ...
    {if (in.is_open())
    {
        in >> y0_anti(i) >> x(i);
        //v0_anti
        v0_anti(i) = koeff * (1/pow(cosh(x(i)-2),2)); // проблемы с корнем v/pow((1-v*v), 0.5)
    ...
    */ 
    ...
    //основной цикл antikink
    for(int i=0; i<Iteration; i++)
    {
    VY = Runge_Kutta(tsY_anti, tsV_anti, N, h, ts);
    tsV_anti = tsV_anti + VY.first;
    tsY_anti = tsY_anti + VY.second;
    }
