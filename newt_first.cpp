...
int main()
{
    ...
    F(0) = -1 - 2*y(0) + y(1) - h * h * 2 * y(0) * (y(0) * y(0) -1); // первая минус 1 соответствует у(a_left)
    F(N-1) =  y(N-2) - 2*y(N-1) + 1 - h * h * 2 * y(N-1) * (y(N-1) * y(N-1) -1); // первая + 1 соответствует у(b_right)
    
    for(int i = 1; i< N-1; i++)
    {
        F(i) = y(i-1) - 2*y(i) + y(i+1) - h * h * 2 * y(i) * (y(i) * y(i) -1);
    }
        
    for(int i = 0; i< N; i++) //индекс 0 соответствует у(a_left + h)
    {
        dg(i) = h*h * (6* y(i) * y(i) - 2);
    }
...
for(int i =0; i < Iteration; i++)
{
    ...
    //пересчет вектора ответов
    F(0) = -1 - 2*y(0) + y(1) - h * h * 2 * y(0) * (y(0) * y(0) -1); // первая минус 1 соответствует у(a_left) = bound_left
    F(N-1) =  y(N-2) - 2*y(N-1) + 1 - h * h * 2 * y(N-1) * (y(N-1) * y(N-1) -1); // первая + 1 соответствует у(b_right)
